...
foreach($info->orderLine as $key => $d){
    ...
    $detail[$key]['money_sale_off'] = $d->quantity * $d->price * $d->discount / 100;
...
echo '<pre>',print_r($info->deliver_to,1),'</pre>';
echo '<pre>',print_r($info->customer_name,1),'</pre>';
echo '<pre>',print_r($info->customer_phone,1),'</pre>';
echo '<pre>',print_r($info->costcenter,1),'</pre>';
echo '<pre>',print_r($detail,1),'</pre>';
echo '<pre>',print_r($start,1),'</pre>';
echo '<pre>',print_r($info->deliver_to,1),'</pre>';
echo '<pre>',print_r($info->pre_pay,1),'</pre>';
echo '<pre>',print_r($info->note,1),'</pre>';
echo '<pre>',print_r($tiencoc,1),'</pre>';
...
curl_setopt_array($dh, array(
  CURLOPT_URL => 'https://zip.1office.vn/api/sale/purchase/insert?access_token=123561329260618a3ac2874&currency_unit=VND&code='.$info->so_number.'&date_sign='.$start.'&customer_id='.$info->deliver_to.'&total_price_internal='.$info->pre_pay.'&detail='.json_encode($detail).'&payment='.json_encode($tiencoc).'&inventory_id='.$kho.'&vat_id='.$vat,
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => '',
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 0,
...
</script><?php /**PATH /home/noith733/zip/resources/views/office.blade.php ENDPATH**/ ?>