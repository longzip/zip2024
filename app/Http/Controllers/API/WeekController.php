...
class WeekController extends Controller
{
	...
	public function showroom(Request $request)
	{
        ...
        // return $listcos->pluck('id');
        if ($checksale) {
	        foreach ($listcos as $key => $value) {
	        	$showroom[$value['name']] = $sr[$value['name']];
	        }
	        $cos = Costcenter::whereIn('id', $listcos->pluck('id'))->where('id','!=',39)->orderBy('pos','asc')->get();
	        // return $showroom;
        }else{
        	if ($request->costcenters) {
...
// dd(count($request->costcenters));
        if (Costcenter::whereDate('closed','2090-01-01')->count() === count($numbersr) + 1) {
	        // $tongcolumn['Tổng']['khtn'] = $p11;
	        $chay = 1;
	        // $tongcolumn['Tổng']['tachkhtn'] = $p12 - $p11;
	        // $tongcolumn['Tổng']['kh15'] = $k1->count();
